...
const getCategoryIcon = (categoryName: string) => {
  ...
  // Return appropriate SVG path based on category name
  if (name.includes('breakfast') || name.includes('morning')) {
    // Breakfast icon (coffee cup)
    return <path d="M2 21v-2h18v2H2zM20 8V5h-2v3h2zM6 8V5H4v3h6V5H8v3H6zM4 9v5h16V9H4z M4 19h16v-3H4v3z" />;
  } else if (name.includes('lunch') || name.includes('main')) {
    // Lunch icon (plate with fork and knife)
    return <path d="M8.1 13.34l2.83-2.83L3.91 3.5c-1.56 1.56-1.56 4.09 0 5.66l4.19 4.18zm6.78-1.81c1.53.71 3.68.21 5.27-1.38 1.91-1.91 2.28-4.65.81-6.12-1.46-1.46-4.2-1.1-6.12.81-1.59 1.59-2.09 3.74-1.38 5.27L3.7 19.87l1.41 1.41L12 14.41l6.88 6.88 1.41-1.41L13.41 13l1.47-1.47z" />;
  } else if (name.includes('dinner') || name.includes('evening')) {
    // Dinner icon (dining table)
    return <path d="M2 19h20v2H2v-2zm9-11h2v8h-2zM5 8h2v8H5zm12 0h2v8h-2zM2 3v2h20V3H2z" />;
  } else if (name.includes('dessert') || name.includes('sweet') || name.includes('cake')) {
    // Dessert icon (cake)
    return <path d="M12 6c1.11 0 2-.9 2-2 0-.38-.1-.73-.29-1.03L12 0l-1.71 2.97c-.19.3-.29.65-.29 1.03 0 1.1.9 2 2 2zm4.6 9.99l-1.07-1.07-1.08 1.07c-1.3 1.3-3.58 1.31-4.89 0l-1.07-1.07-1.09 1.07C6.75 16.64 5.88 17 4.96 17c-.73 0-1.4-.23-1.96-.61V21c0 .55.45 1 1 1h16c.55 0 1-.45 1-1v-4.61c-.56.38-1.23.61-1.96.61-.92 0-1.79-.36-2.44-1.01zM18 9h-5V7h-2v2H6c-1.66 0-3 1.34-3 3v1.54c0 1.08.88 1.96 1.96 1.96.52 0 1.02-.2 1.38-.57l2.14-2.13 2.13 2.13c.74.74 2.03.74 2.77 0l2.14-2.13 2.13 2.13c.37.37.86.57 1.38.57 1.08 0 1.96-.88 1.96-1.96V12C21 10.34 19.66 9 18 9z" />;
  } else if (name.includes('drink') || name.includes('beverage')) {
    // Drinks icon (glass)
    return <path d="M3 2l2.01 18.23C5.13 21.23 5.97 22 7 22h10c1.03 0 1.87-.77 1.99-1.77L21 2H3zm9 17c-1.66 0-3-1.34-3-3 0-2 3-5.4 3-5.4s3 3.4 3 5.4c0 1.66-1.34 3-3 3zm6.33-11H5.67l-.44-4h13.53l-.43 4z" />;
  } else if (name.includes('appetizer') || name.includes('starter')) {
    // Appetizer icon (small plate)
    return <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-5-9h10v2H7z" />;
  } else if (name.includes('salad') || name.includes('vegetable') || name.includes('vegan')) {
    // Salad/Vegetable icon (leaf)
    return <path d="M6.05 8.05c-2.73 2.73-2.73 7.17 0 9.9C7.42 19.32 9.21 20 11 20s3.58-.68 4.95-2.05C19.43 14.47 20 4 20 4S9.53 4.57 6.05 8.05zm8.49 8.49c-.95.94-2.2 1.46-3.54 1.46-1.34 0-2.59-.52-3.54-1.46-1.95-1.96-1.95-5.12 0-7.07.97-.97 2.25-1.46 3.54-1.46 1.28 0 2.56.49 3.54 1.46 1.95 1.95 1.95 5.11 0 7.07z" />;
  } else if (name.includes('seafood') || name.includes('fish')) {
    // Seafood icon (fish)
    return <path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-10.5h2v5h-2zm0-4h2v2h-2z" />;
  } else if (name.includes('pizza')) {
    // Pizza icon
    return <path d="M12 2C8.43 2 5.23 3.54 3.01 6L12 22l8.99-16C18.78 3.55 15.57 2 12 2zm0 15.92L5.51 6.36C7.32 4.85 9.62 4 12 4s4.68.85 6.49 2.36L12 17.92zM9 5.5c-.83 0-1.5.67-1.5 1.5S8.17 8.5 9 8.5s1.5-.67 1.5-1.5S9.82 5.5 9 5.5zm1.5 7.5c0 .83.67 1.5 1.5 1.5.82 0 1.5-.67 1.5-1.5s-.68-1.5-1.5-1.5-1.5.67-1.5 1.5z" />;
  } else {
  ...
  return (
    <div className="relative mb-6">
      <div className="flex items-center">
        ...
        <div className="relative flex-grow overflow-hidden">
          ...
          >
            ...
            <button
              ...
              className="flex items-center gap-3 bg-white text-black px-4 py-3 rounded-xl shadow-sm hover:shadow-md transition-shadow whitespace-nowrap"
            >
              {/* Colored icon container */}
              <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-[#f4cadf]">
              ...
              {/* Text content */}
              <span className={`text-base ${selectedCategory === null ? 'font-bold' : 'font-medium'}`}>All Categories</span>
            </button>
            ...
            {categories.map((category) => (
              ...
              >
                ...
                {/* Text content */}
                <span className={`text-base ${selectedCategory === category.id ? 'font-bold' : 'font-medium'}`}>
        ...
        >
          {/* Colored icon container */}
          <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-[#f4d0ca]">
